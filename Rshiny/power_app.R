...
server <- function(input, output,session) {
  ...
  output$newtrt_size <- renderUI({
    if(input$newtrt == "arisk"){
      tagList(
        numericInput("risk3",paste0("Risk of the new treatment"),
                     step=0.000001,value = 0.30,max = 0.99999,min = 0.00001)
      )
    }else{
      tagList(
  ...
  output$tabOutput <- function() {
    ...
    if(!is.null(dat)){
      ...
      samplesize_even = rep(SolveSampleSize_Withprev_equal(risk1,risk2,risk3,sigma,power_level)/3,3)
      samplesize = SolveSampleSize_Withprev(risk1,risk2,risk3,sigma,power_level)
      #samplesize_single = SolveSampleSize_Single(risk1,risk2,power_level)
      samplesize_single_even = rep(SolveSampleSize_Single_equal(risk1,risk2,risk3,power_level)/3,3)
      ...
      collapse_rows_dt <- cbind(C1 = c(rep("with previous NMA", 2), "isolation"),
